...
async def help_message_handler(message, COMMAND_PREFIX):
    try:
        ...
        await message.channel.send(embed=await text_module.embeds.embed_error_message("Something went wrong whilst trying to execute that command."))
    ...
    if second_parameter == "text":
        ...
        await message.channel.send(embed=await text_module.embeds.embed_error_message("Must specify valid help message."))

async def default_help_message(message, COMMAND_PREFIX):
    await message.channel.send(embed=await text_module.embeds.embed_response_custom_emote("Commands:", f'''
...
async def image_help_message(message, COMMAND_PREFIX):
    await message.channel.send(embed=await text_module.embeds.embed_response_custom_emote("Image Commands:", f'''
