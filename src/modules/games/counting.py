...
async def start_counting(guild_id, message, increment=1):
    await message.channel.send(
        embed=verbose.embeds.embed_response(
            "Your counting game has started.", f"Type `{increment}` to start"
        )
    )
...
async def check_message(message):
    ...
    if guild_id in counting_data.keys():
        ...
        if (
            counting_data[guild_id]["counting_has_started"]
            and message_float is not None
        ):
            ...
            if (
                len(messages_list_for_guild) > 1
                and messages_list_for_guild[len(messages_list_for_guild) - 1].author.id
                == messages_list_for_guild[len(messages_list_for_guild) - 2].author.id
            ):
                await message.add_reaction("❌")
                await message.channel.send(
                    embed=verbose.embeds.embed_failed_counting(
                        "Counting twice in a row is no fun.",
                        f"You counted up to {counting_data[guild_id]['next_number'] - counting_data[guild_id]['increment']}",
                    )
                )
            ...
            if message_float == counting_data[guild_id]["next_number"]:
                counting_data[guild_id]["next_number"] += counting_data[guild_id][
                    "increment"
                ]
                await message.add_reaction("✅")
            else:
...
async def send_stats(message, messages_list_for_guild):
    ...
    await message.channel.send(
        embed=verbose.embeds.embed_response("Counting Stats:", stats_message)
    )
