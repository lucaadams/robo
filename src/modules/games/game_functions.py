...
async def start_game(guild_id, message):
    ...
    try:
        ...
        await message.channel.send(embed=verbose.embeds.embed_error_message("Must specify a game."))
        return
...
async def set_increment(message):
    ...
    try:
        ...
        await message.channel.send(embed=verbose.embeds.embed_error_message("Increment must be an number. Game will continue with increment `1`."))
    except IndexError:
        pass
