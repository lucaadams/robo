    ...
    if second_parameter == "add":
        ...
        try:
            ...
            await message.channel.send(
                embed=verbose.embeds.embed_error_message(
                    "That keyword does not exist. Did you make a typo? "
                )
            )
        ...
        try:
            if message.content.split(" ")[3] == "":
                await message.channel.send(
                    embed=verbose.embeds.embed_error_message(
                        "Name of new keyword must be specified. "
                    )
                )
            else:
                await edit(
...
async def add(guild_id, message, keyword, value):
    ...
    if value == "":
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("No value specified. ")
        )
    else:
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_successful_action("Keyword added. ")
        )
...
async def remove(guild_id, message, message_removal):
    ...
    await message.channel.send(
        embed=verbose.embeds.embed_successful_action("Keyword removed. ")
    )
...
async def edit(guild_id, message, old_keyword, new_keyword):
    guild_data = data.get_guild_data(guild_id)
    guild_data["keywords"][new_keyword] = guild_data["keywords"].pop(old_keyword)
    await message.channel.send(
        embed=verbose.embeds.embed_successful_action("Keyword edited. ")
    )
    ...
    if keywords_list == "":
        await message.channel.send(
            embed=verbose.embeds.embed_response(
                "No keywords set.", "Nothing to display."
            )
        )
    else:
        await message.channel.send(
            embed=verbose.embeds.embed_response("Keywords:", f"{keywords_list}")
        )
...
async def check_keywords(guild_id, message):
    ...
    for keyword in guild_data["keywords"].keys():
        if message.content.__contains__(keyword):
            await message.channel.send(
                guild_data["keywords"][keyword][
                    random.randint(0, len(guild_data["keywords"][keyword]) - 1)
                ]
            )
            return
