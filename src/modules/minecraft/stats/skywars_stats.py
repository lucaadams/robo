...
def embed_skywars_stats(base_player_data, page_number) -> discord.Embed:
    ...
    stats_embed.set_author(icon_url=hypixel_logo_url, name=f" {gamemode} skywars stats")
...
def get_overall_skywars_stats(data, player_rank) -> discord.Embed:
    ...
    try:
        overall_stats_data = {
            ...
            "Wins": data["wins"],
            "Losses": data["losses"],
            "WLR": round(data["wins"] / data["losses"], 2),
            "Kills": data["kills"],
            "Deaths": data["deaths"],
            "K/D": round(data["kills"] / data["deaths"], 2),
            "Total souls": data["souls_gathered"],
            "Heads": data["heads"],
            "Chests Opened": data["chests_opened"],
            "Arrows shot": data["arrows_shot"],
            "Arrows Hit": data["arrows_hit"],
            "Accuracy": f"{round((data['arrows_hit']/data['arrows_shot']) * 100, 2)}%",
        }
    except:
...
def get_mode_specific_skywars_stats(data, player_rank, gamecode) -> discord.Embed:
    ...
    try:
        overall_stats_data = {
            ...
            "Coins": data["coins"],
            "Winstreak": data["win_streak"],
            "Wins": data[f"wins_{gamecode}"],
            "Losses": data[f"losses_{gamecode}"],
            "WLR": round(data[f"wins_{gamecode}"] / data[f"losses_{gamecode}"], 2),
            "Kills": data[f"kills_{gamecode}"],
            "Deaths": data[f"deaths_{gamecode}"],
            "K/D": round(data[f"kills_{gamecode}"] / data[f"deaths_{gamecode}"], 2),
        }
    except:
        raise StatsNotFoundError
