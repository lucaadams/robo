...
class Quote:
    ...
    def __init__(
        ...
        self.quote_message_wrap = wrap(self.quote_message, self.max_chars_per_line)
    ...
    async def generate_quote(self):
        ...
        image_with_message.text(
            (self.quote_location_x, self.quote_location_y),
            self.quote_content,
            (self.font_colour, self.font_colour, self.font_colour),
            font=self.font,
        )
...
async def execute_quote_command(message):
    ...
    if image_type == "colour":
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_error_message(
                "You must specify a valid image type ('grey' or 'colour')"
            )
        )
