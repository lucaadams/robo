...
async def vc_command_handler(message):
    ...
    # if no second parameter specified, send error message
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("Incomplete command.")
        )
        return
    ...
    if second_parameter == "join":
        ...
        if (
            len(guild_vc_data[guild_id]["guild_queue"]) == 1
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_response_without_title_custom_emote(
                f"Queue shuffled.", ":twisted_rightwards_arrows:"
            )
        )
        ...
        if guild_vc_data[guild_id]["enable_np"]:
            ...
            await message.channel.send(
                embed=verbose.embeds.embed_response_without_title_custom_emote(
                    "'Now playing' message disabled.", ":ok_hand:"
                )
            )
            ...
            await message.channel.send(
                embed=verbose.embeds.embed_response_without_title_custom_emote(
                    "'Now playing' message enabled.", ":ok_hand:"
                )
            )
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("Invalid command.")
        )
...
async def join_voice_channel(message):
    ...
    # if already in a channel, catch error and reply on discord
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_sorry_message("I am already in a voice channel.")
        )
        return
...
async def leave_voice_channel(message):
    ...
    try:
        if not guild_vc_data[guild_id]["voice_client"].is_connected():
            await message.channel.send(
                embed=verbose.embeds.embed_sorry_message(
                    "I am not currently in any voice channel."
                )
            )
...
async def play_from_yt(message):
    ...
    try:
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_response(
                "The queue is empty.",
                "I will stay in the voice channel... in silence...",
            )
        )
        return
    ...
    audio = pafy.new(
        song_request_metadata["id"], ydl_opts=YOUTUBE_DL_OPTIONS
    ).getbestaudio()
    voice_client.play(
        discord.FFmpegPCMAudio(audio.url, options=FFMPEG_OPTIONS),
        after=lambda e: asyncio.run_coroutine_threadsafe(
            on_playback_finished(message), bot.CLIENT.loop
        ),
    )
...
async def on_playback_finished(message):
    ...
    if not guild_vc_data[guild_id]["loop"]:
        try:
            ...
            await message.channel.send(
                embed=verbose.embeds.embed_response(
                    "Your queue has finished playing.",
                    "I will stay in the voice channel... in silence...",
                )
            )
...
async def add_song_to_queue(message):
    ...
    if len(user_song_request) < 1:
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("No request specified.")
        )
        return
    ...
    await message.channel.send(
        embed=verbose.embeds.embed_successful_action(
...
async def remove_from_queue(message):
    ...
    try:
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("Must specify valid queue index.")
        )
        return
    ...
    try:
        await message.channel.send(
            embed=verbose.embeds.embed_successful_action(
                f"[{guild_vc_data[guild_id]['guild_queue'][index_to_remove - 1]['title']}]({guild_vc_data[guild_id]['guild_queue'][index_to_remove - 1]['webpage_url']}) \
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("That queue index does not exist.")
        )
        return
...
async def send_queue(message):
    ...
    if len(specific_queue) == 0:
        ...
        if desc == "":
            ...
            await message.channel.send(
                embed=verbose.embeds.embed_response("Up next", desc)
            )
        return
    ...
    if specific_queue in guild_data["saved_queues"]:
        ...
        if desc == "":
            await message.channel.send(
                embed=verbose.embeds.embed_response_without_title(
                    "That queue is currently empty."
                )
            )
        else:
            await message.channel.send(
                embed=verbose.embeds.embed_response(f"Queue `{specific_queue}`:", desc)
            )
        return

    await message.channel.send(
        embed=verbose.embeds.embed_error_message(
            "You do not have a saved queue with that name."
        )
    )
...
async def send_queue_list(message):
    ...
    for queue_name in guild_data["saved_queues"]:
        ...
        queue_duration = (
            f"{time_hours}h {time_mins}m {time_secs}s"
            if time_hours != 0
            else f"{time_mins}m {time_secs}s"
        )
...
async def play_queue(message):
    ...
    if queue_to_play not in guild_data["saved_queues"]:
        await message.channel.send(
            embed=verbose.embeds.embed_error_message("That queue does not exist.")
        )
        return

    guild_vc_data[guild_id]["guild_queue"] = guild_data["saved_queues"][
        queue_to_play
    ].copy()
    ...
    try:
        ...
        await message.channel.send(
            embed=verbose.embeds.embed_response(
                "Queue set.", "Type `!robo vc join` to start listening."
            )
        )
        return
...
async def save_queue(message):
    ...
    if len(guild_vc_data[guild_id]["guild_queue"]) < 1:
        await message.channel.send(
            embed=verbose.embeds.embed_sorry_message("Your current queue is empty.")
        )
        return

    queue_name = " ".join(message.content.split(" ")[3:])
    guild_data = data.get_guild_data(guild_id)
    guild_data["saved_queues"][queue_name] = guild_vc_data[guild_id][
        "guild_queue"
    ].copy()
