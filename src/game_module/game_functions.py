...
async def start_game(guild_id, message):
    ...
    try:
        ...
        await message.channel.send(embed=await text_module.quote_functions.embed_error_message("Increment must be an number. Game will continue with increment `1`."))
    except IndexError:
        pass

    if game == "counting":
        await game_module.counting.start_counting(guild_id, message, increment)
    else:
